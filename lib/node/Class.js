...
function Class(className, descriptor) {
    ...
    if (registryEntry) {
        logger.info("Compiled class is already in registry. Taking shortcut ...");
    ...
    logger.info("Executing compiled source ...");
    constructor = executeCompiledSrc(compiledSrc, descriptor, className);
    registry.setEntry(descriptor, constructor, compiledSrc);

    logger.info("Compilation of " + className + " took " + (new Date().getTime() - start.getTime()) + "ms");
...
function executeCompiledSrc(compiledSrc, descriptor, className) {
    ...
    require("fs").writeFileSync("C:/_" + className + ".js", compiledSrc, "utf8");
