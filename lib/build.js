...
var _PropertiesLeftHand = 'Properties.<%= key %>', // used by every Properties-assignment on the left hand
    _FunctionRightHand = 'Class.<%= key %>.bind(Properties)', // used by function assignments on the right hand
    ...
    _StaticsLeftHand = 'Constructor.<%= key %>', // left hand assignment for static properties
    _StaticsRightHand = 'Class.<%= key %>', // right hand assignment for static properties
    _Init = 'Class.<%= initName %>.apply(Properties,arguments);', // executes the init-function. requires the init name since there are two possible names
    ...
    _Code = // the Constructor-function template
        'var Module=this;'
        + 'var Constructor=function(){' // constructor definition begins here
            + 'var tmp,'
            + 'implChildAbstracts,'
            + 'implAbstracts,'
            ...
            + 'Properties,'
            + 'key;'
            + 'if(Constructor.$){'  // reads the flag-object. these flags are used to modify the constructor's behaviour.
                + 'tmp=Constructor.$;'
            ...
            + 'if(constructorIsRunning){' // if the constructor is currently running, it looks like it's trying to call itself. therefore a recursion error is thrown.
                ...
                + 'return;'
            + '}else{'
                + 'Constructor.$={"constructorIsRunning":true};'
            + '}'
...
var initConstructor
        ...
        = 'Constructor.$init=function(){'
            + 'if(Extends.$init){'
                + 'Extends.$init();' // recursive call of the InitConstructor function of the super class.
            + '}'
            + 'Extends.$={'
                + '"exposeNothing":true'
                + '};'
            + 'Constructor.prototype=new Extends();'
            + 'delete Constructor.$init;' // after execution, this function deletes itself since it is not used anymore
        + '};',
    superConstructor =
    ...
    superConstructorCall =
        ...
        'if(typeof Properties.Super==="function"){'
            + 'Properties.Super.apply(Properties.Super,arguments);'
        + '}';
    ...
    _(Class).each(function eachProperty(value, key) {
        ...
        if(/^init(ialize)?$/.test(key)) {   // IF TRUE: init function
            if(result.Init) {   // IF TRUE: There is already an init function stored in the result object
                throw new Error('build error in ' + classFile + ': found two init methods "' + key + '" and "' + result.Init + '".');
            } else if(typeof value !== 'function') {    // IF TRUE: the init property is not a function
                throw new Error('build error in ' + classFile + ': the init method "' + key + '" is not a function.');
            }
            result.Init = key;
        } else if(prefix === '?') { // IF TRUE: abstract property
            if(typeof value !== 'function') {
                throw new Error('build error in ' + classFile + ': you can only define abstract functions.\n'
 ...
 * @returns {String} source
 */
function doBuild(file, classModule) {
    var This,
        Super,
        src,
        implAbstracts;
    ...
    // the classFile variable is trimmed. Only the class path after
    // the last "node_modules" is kept.
    classFile = classFile.replace(/.*node_modules\//gi, '');
        ...
        _(Super.Abstract).each(function eachAbstracts(value, key) {
            ...
            if(classModule.Class.hasOwnProperty(key)) {
                if(typeof classModule.Class[key] === 'function') {
                    ...
                    throw new Error('build error in ' + file + ': the implemented abstract property "' + key + '" is typeof ' + typeof classModule.Class[key] + ' instead of being typeof function.\n'
                        + 'you can only define abstract methods.');
                }
                delete Super.Abstract['?' + key];
            }
        });
        if(_(Super.Abstract).size() > 0) {
            throw new Error('build error in ' + file + ': you didnt take care of the inherited abstracts function(s) "' + _(Super.Abstract).keys().join('", "') + '".\n'
