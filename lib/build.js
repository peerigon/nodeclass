 ...
 * @param {String} classFile path to the file
 * @returns {String} source
 */
function build(classModule, classFile) {
    var This,
        Super,
        src;
        
    if(!classFile) {
        classFile = 'Anonymous Class';
    }
    ...
    if(typeof classModule !== 'object') {
        throw new Error('build error in ' + classFile + ':\nthe class module is not an object. instead of its typeof ' + typeof classModule);
    }
    ...
    try {
        ...
        src = assembleStrings(This, classFile, Super);
    } catch(err) {
        throw new Error('build error in ' + classFile + ': ' + err.message);
    }
