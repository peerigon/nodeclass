 ...
 * @returns {Object} result
 */
function collectSuperProperties(Class) {
    var result;

    if(Class.Extends) {    // IF TRUE: this class has a super class. so we're collecting these properties first.
        result = collectSuperProperties(Class.Extends);
        result = collectProperties(Class, result);
    } else {
