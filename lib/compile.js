...
function load(path) {
    ...
    if (!loaded[path] && /\.class\.js$/gi.test(path)) {
        src = classesToLoad[path];
        vm.runInNewContext(src, sandbox, path);
        if (typeof sandbox.module.exports !== "object") {
            throw new Error("(nodeclass) Cannot compile class '" + path + "': Module doesn't export an object.");
        }
        loaded[path] = sandbox.module.exports;
    }
...
function buildClasses() {
    ...
    for (path in classesToLoad) {
        if (classesToLoad.hasOwnProperty(path)) {
            src = classesToLoad[path];
            module = load(path);
            main.stdout('building ' + path);
            try {
                src += '\n\n\n' + build(module);
            } catch(e) {
                e.msg = 'An error occured while building class ' + path + '.\n' + e.msg;
                throw e;
            }
...
function compile(srcPath, compilePath, compileOptions) {
    ...
    main.stdout('reading files from ' + srcPath);
    ...
    buildClasses();
    main.stdout("removing compiled folder");
    removeSync(compilePath);
    main.stdout('writing files to ' + compilePath);
    writeSync(compilePath, classes, compileOptions.encoding);
    writeSync(compilePath, modules, compileOptions.encoding);

    main.stdout('finished compiling');
}
